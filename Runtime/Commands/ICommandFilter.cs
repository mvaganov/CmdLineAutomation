namespace RunCmd
{
	public interface ICommandFilter : ICommandProcessor {
		...
		/// <see cref="ICommandProcess"/>. The command modifies the input command into an output
		/// command, like a filter. <see cref="FilterResult(object)"/> resolves to the filtered value
		/// when the command is finished executing (<see cref="ICommandProcess.IsExecutionFinished"/>)
		/// </summary>
