	...
	public class CommandAssetGetChoice : ScriptableObject, ICommandAsset {
		...
		public class Proc : BaseNamedProcess {
			...
			public override float GetProgress() {
				...
				if (source._blockUntilChoiceIsMade) {
					return 0;
				}
				int waited = Environment.TickCount - timeoutStart;
				float normalizedProgress = (float)waited / timeoutDuration;
			...
			public override void StartCooperativeFunction(string command, PrintCallback print) {
				...
				string message = (arguments.Count > 1) ? (Parse.Token)arguments[1] : (Parse.Token)source._defaultMessage;
				IDictionary args = (arguments.Count > 2) ? arguments[2] as IDictionary : source.DefaultChoiceDictionary();
				...
				if (source._blockUntilChoiceIsMade) {
					...
					timeoutDuration = (int)(source._choiceTimeout * 1000);
					finished = false;
					onChoiceMade = (choice) => source.ChoiceMade(context, choice);
				}
				int entryIndex = 0;
				...
				if (!Application.isPlaying) {
					choiceWindow = GetChoiceWindow.Dialog(message, argsOptions,
						argsActions, onChoiceMade, size, size / -2, source._blockingUnderlay);
					choiceWindow.rootVisualElement.style.backgroundColor = source._choiceWindowColor;
					if (source._blockingUnderlay) {
						choiceWindow._choiceBlocker.rootVisualElement.style.backgroundColor = source._underlayWindowColor;
					}
					return;
				}
#endif
				...
				if (ComponentGetChoice.Instance != null) {
					runtimeComponent = ComponentGetChoice.Instance;
					runtimeComponent.Message = message;
					runtimeComponent.SetChoices(argsOptions, onChoiceMade, context, source);
					GameObject choiceGameObject = runtimeComponent.ChoiceWindow.gameObject;
					Debug.Log(choiceGameObject);
					UnityEngine.UI.Image img = choiceGameObject.GetComponent<UnityEngine.UI.Image>();
					img.color = source._choiceWindowColor;
					choiceGameObject.SetActive(true);
					if (source._blockingUnderlay) {
						runtimeComponent.Underlay.GetComponent<UnityEngine.UI.Image>().color = source._underlayWindowColor;
					}
					runtimeComponent.Underlay.gameObject.SetActive(source._blockingUnderlay);
					runtimeComponent.gameObject.SetActive(true);
				}
			}
		}
...
//			}
		...
		public void ChoiceMade(object context, int choiceIndex) {
			...
			if (proc == null) {
				...
				//this.GetExecutionData(context);
			proc.finished = true;
#if UNITY_EDITOR
			proc.choiceWindow?.CloseChoiceWindow();
#endif
			if (ComponentGetChoice.Instance != null && proc.runtimeComponent != null) {
				proc.runtimeComponent.gameObject.SetActive(false);
			}
		}
