...
namespace RunCmdRedux {
	...
	public class AutomationExecutor : ICommandAssetExecutor, ICommandAssetAutomation, ICommandProcessReference {
		...
		public void ExecuteCurrentCommand() {
			...
			OutputAnalysis($"executing \"{currentCommandText}\"\n");
			...
			// TODO test me out!
			_settings.Execute(currentCommandText, OutputAnalysis, this, FinishedSingleCommand);

			//StartCooperativeFunction(currentCommandText, print);
		}

		private void FinishedSingleCommand(object result) {
			if (IsCancelled()) {
				Debug.Log("----------CANCELLED "+result);
				EndCurrentCommand();
				return;
			}
